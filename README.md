...
```text
7   -> цифра, роль берётся из config.json
T7  -> десятки 7
U7  -> единицы 7
O7  -> единицы 7
```
...
```text
Arduino десятков отправляет: T1
Arduino единиц отправляет: U7
...
Если `led.enabled=false`, приложение не отправляет команды и работает без ленты. Для текущего стенда: `0` выключает подсветку, `2` запускает анимацию от барабана к экрану для годов `2016-2026` и держит её до окончания ролика, `1` зажигает линии для годов вне диапазона на `outOfRangeDurationMs`, затем приложение отправляет `0`.
