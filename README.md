...
Если `led.enabled=false`, приложение не отправляет команды и работает без ленты. Для текущего стенда: `0` выключает подсветку, `2` запускает анимацию от барабана к экрану для годов `2016-2026` до окончания ролика, `1` зажигает линии для годов вне диапазона на `outOfRangeDurationMs`, затем приложение отправляет `0`.
